...
export const PreviewPanel = ({ state, onSave, isSaving, onDismissError }: PreviewPanelProps) => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex-1 flex items-center justify-center bg-gray-950/50 p-6 overflow-auto">
        ...
        {state.status === 'error' && (
          <ErrorState message={state.message} onDismiss={onDismissError} />
        )}
        {state.status === 'success' && (
          activeTab === 'preview' ? (
...
const ErrorState = ({
  ...
  <div className="text-center max-w-md px-4">
    ...
    <p className="text-sm text-red-400 whitespace-pre-wrap break-words">{message}</p>
    <p className="text-xs text-gray-500 mt-2">
      For 429 errors, fix billing on your OpenAI account; the app cannot bypass API limits.
    </p>
    {onDismiss && (
      <button
        type="button"
        onClick={onDismiss}
        className="mt-4 text-xs px-3 py-1.5 bg-gray-800 text-gray-300 rounded-lg border border-gray-700 hover:text-white hover:border-gray-600 transition-colors"
      >
        Dismiss
      </button>
    )}
