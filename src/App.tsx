...
const formatGenerationError = (err: unknown): string => {
  if (err instanceof APIError) {
    if (err.status === 429) {
      return `Quota or rate limit exceeded (429). Add credits or check your plan: ${OPENAI_BILLING}`;
    }
    if (err.status === 401) {
      return 'Invalid API key (401). Clear the saved key and paste a new one from OpenAI.';
    }
...
export const App = () => {
  ...
  const handleGenerate = useCallback(async (prompt: string) => {
    ...
    try {
      ...
      setGenerationState({ status: 'error', message: formatGenerationError(err) });
    }
  }, [apiKey]);
  ...
  return (
    <div className="flex h-screen bg-gray-950 text-white overflow-hidden">
      ...
      <PreviewPanel
        ...
        onSave={handleSave}
        isSaving={isSaving}
        onDismissError={() => setGenerationState({ status: 'idle' })}
      />
      ...
      <VariantsSidebar
        ...
        onDismissError={() => setGalleryState({ status: 'success', components: [] })}
      />
    </div>
  );
};
